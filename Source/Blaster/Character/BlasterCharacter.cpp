...
ABlasterCharacter::ABlasterCharacter()
{
	...
	// Used only on the server so no need to make it replicated
...
void ABlasterCharacter::SpawnDefaultWeapon()
{
	...
	if (BlasterGameMode && World && !bElimmed && DefaultWeaponClass) // We know we're on the server AND we know we are using the blaster gamemode, not lobby gamemode
	{
