...
AWeapon::AWeapon()
{
	...
	WeaponMesh->MarkRenderStateDirty(); // forces a refresh 
...
{
	// Updating the HUD ammo for the character holding this weapon (the owner of this weapon)
...
void AWeapon::OnRep_Owner()
{
	...
	else
	{
		...
		if (BlasterOwnerCharacter && BlasterOwnerCharacter->GetEquippedWeapon() && BlasterOwnerCharacter->GetEquippedWeapon() == this) // making sure that this weapon is the equipped weapon and then updating the HUD
		{
			SetHUDAmmo();
		}
...
void AWeapon::OnEquipped()
{
	...
	BlasterOwnerCharacter = BlasterOwnerCharacter == nullptr ? Cast<ABlasterCharacter>(GetOwner()) : BlasterOwnerCharacter;
	if (BlasterOwnerCharacter && bUseServerSideRewind) // If bUseServerSideRewind is false then there's no need to bind to this delegate
	{
		BlasterOwnerController = BlasterOwnerController == nullptr ? Cast<ABlasterPlayerController>(BlasterOwnerCharacter->Controller) : BlasterOwnerController;
...
// Called on all clients and server from multicast
void AWeapon::Fire(const FVector& HitTarget)
{
	...
	if (CasingClass)
	{
		...
		if (AmmoEjectSocket)
		{
			...
			// Our bullet shell is not affecting anything in the game (damage, etc) so there is no need to set owner or instigator, so no need for SpawnParams
